...
## 2026-08-08 (evening 2) — sprint complete: audition + export (PRs #21/#22); todo list 5/5
- **live_play_notes** ([#21](https://github.com/bschoepke/ableton-live-mcp/pull/21)): real-time audition through an AgentM4L midi_effect — js outlet 3 + play_notes scheduler, patch chain js→makenote→pack→midiformat→midiout. First wiring was silently wrong: **midiformat inlet 1 is poly pressure, not velocity** (velocity-0 note-ons = silence); the pack fix is in-Live verified — Em arps through Air Flute Magnus, no clip, transport stopped. New gotcha found by Kim's ears: **a freshly (re)loaded device replays the last command-file entry** (dedupe id resets; heard as tripled arps). Follow-up: prime lastCommandId at loadbang without executing — carefully, around ensureRecovered.
- **live_export** ([#22](https://github.com/bschoepke/ableton-live-mcp/pull/22), stacked on #18): drives Ctrl+Shift+R → confirm (last-used settings) → types path into the native save dialog → polls for a size-stable, PARSEABLE wav. In-Live verified: 32-beat render = 16.27 s at 118 BPM exactly, 24-bit/44.1k inherited, then live_analyze read it back at −19.8 LUFS — **the mastering loop is fully closed** (compose → mix → master → export → measure, no human required except taste).
- Second test-isolation gap found: the validate suite scans the REAL install folders (agent_m4l_host_status), so an AgentM4L host installed from a newer branch makes upstream-based branches' validate tests fail. Both isolation gaps (state dir + install scan) are one conftest fix — upstream-worthy.
- Upstream queue now #14–#22 (nine PRs). Todo list from Kim's "what's obviously missing" question: **5/5 shipped and in-Live verified in one day.**

## 2026-08-08 (night) — capability sprint: save, undo, analyze (PRs #18/#19/#20)
- Kim asked what's obviously missing; built the top three, each on its own branch from upstream/main, PR'd upstream, merged to kim/main, and verified in-Live:
  - **live_save_set** ([#18](https://github.com/bschoepke/ableton-live-mcp/pull/18)): OS-level Ctrl+S (ctypes SendInput; osascript on mac) verified via .als mtime — the LOM has no save API or dirty flag. Refuses a never-saved set. In-Live: refusal path + two verified saves. Learned: an undirty set skips the rewrite on Ctrl+S entirely.
  - **live_undo** ([#19](https://github.com/bschoepke/ableton-live-mcp/pull/19)): N-step undo/redo via eval/exec, exhaustion-safe; checkpoint pattern documented. In-Live: rename → undo → name restored.
  - **live_analyze** ([#20](https://github.com/bschoepke/ableton-live-mcp/pull/20)): analyze a wav or tap-capture first; BS.1770-4 integrated LUFS + peak + band energies + dominant Hz; numpy via new [analyze] extra. Validated against the spec's sine calibration (dual-mono full-scale = 0 LUFS ref — NOT −3; that figure is single-channel). In-Live: sketch loop measured −29.1 LUFS / −14.2 dBFS peak.
- Field observations: (a) the tap dropped the FIRST command after ~1h idle (start acknowledged nothing; second attempt clean — watch item; agents should handshake-probe before captures); (b) fake-live tests read/write the REAL state dir (~/.ableton-live-mcp), so a concurrent live session can flake them — test isolation via ABLETON_MCP_STATE_DIR env in conftest would fix; (c) tool-list ceiling on kim/main now 21800.
- Remaining wishlist: real-time note audition via AgentM4L (MIDI into a track without clips), live_export (UI-driven render; reuses save's keystroke plumbing).
...
- The session MCP server exe runs pre-handshake code until next Claude Code restart; bridge + device are current now.
- **All contributions offered to both owners (Kim's ask):** upstream [#16](https://github.com/bschoepke/ableton-live-mcp/pull/16) (clip_notes limit vs encoder max_items, from `fix/clip-notes-limit-vs-encoder-max-items`) + [#17](https://github.com/bschoepke/ableton-live-mcp/pull/17) (tap liveness handshake, adapted to upstream's simpler js, from `fix/agent-audio-tap-liveness-handshake`); lylepmills [PR #1](https://github.com/lylepmills/ableton-live-mcp/pull/1) (their stale tap-js test, + friendly pointer to the handshake work). Both fix branches carry the pre-existing Windows build-test failures that #14 fixes — noted in the PR bodies. Now 4 upstream PRs open: #14 #15 #16 #17.
...
## 2026-08-08 (Mysterious session) — real-world master + export, two focus bugs found
- New set "Mysterious" (Kim's Em-G#m-C#m-Gm progression, extended Em-Cm-Am-B; 100 BPM, 32 bars). Kim chose presets on Spire/DUNE3/Vital/ANA2 (some bring own arp/seq — rewrote Claude ARP to HELD triads feeding the presets' engines). Deep track: score was fine, ANA2 preset was a warped sample stuttering low → undo tool used surgically (undo N + redo to restore, verified per-step; note count as checkpoint). Mastered via native tools: measured -21.2 → Utility+5.0/Limiter -0.75 (Kim's choice) = -13.8 (Spotify), Utility+2.8 = -15.9 (Quest/-16). Delivered Mysterious -14LUFS.wav (-13.8) + Mysterious -16LUFS.wav (-15.9), both live_analyze-verified.
- **BUG (both live_export AND live_save_set): plain SetForegroundWindow loses to Windows focus-stealing prevention** when another window (Explorer) is foreground → keystrokes miss Live, export/save silently no-op. Manual fix that works every time: AttachThreadInput(me,fgThread,True) + ShowWindow(SW_RESTORE) + BringWindowToTop + SetForegroundWindow + detach, then verify GetForegroundWindow()==hwnd BEFORE sending keys. Fold into save_set._send_save_keystroke_windows and export_set._drive_export_dialog_windows; follow-up on PRs #18/#22.
- Finding: Utility/Limiter param changes via the LOM may not mark the doc dirty (save_not_observed after an API-only param round-trip that returned to saved values — harmless: disk already matched). Also confirmed exports measure ~1 dB louder per +1 dB Utility, linear, so target LUFS is one gain move.
- Also: session MCP server runs the save_set build from session start; a mid-session branch's newer code isn't picked up until Claude Code restarts — run current code directly (DirectBridge) to test.

## 2026-08-09 — Mysterious finished (choir + re-master) + repo tidy
- **Final master:** re-mastered Mysterious WITH the two-voice EastWest choir added (the -14 render was pre-choir/stale). Utility Output **+8.40 dB** (value 0.24; mapping value×35=dB) → Limiter → **-16.0 LUFS**, peak -1.0 dBFS, 4:29. Delivered `D:\...\Mysterious Project\Mysterious -16LUFS.wav`.
- **Export gotcha (cost 3 wrong renders, worth a fix/doc upstream):** Live's Export **silently renders whatever Arrangement time-selection exists** — a leftover marking on one track (0–224 of 448) hijacked the render to half length with NO warning, and the `song.loop_start/loop_length` brace does NOT override it (unlike Cubase, which forces an explicit all/selection choice). `export_set` sets the loop brace but that's ignored when a selection is present. **Always verify `duration_s` after export**; if short, a stray Arrangement selection is the cause. TODO: consider clearing/normalising the Arrangement selection in `export_set` before driving the dialog, or at least warn when rendered length ≠ requested.
- **Repo tidy:** collected the loose session client/dev scripts from the temp scratchpad into **`examples/`** (minimal `live.py` client + `analyze_set.py`/`lufs.py` demos + e2e/stdio/wincapture/build/probe/verify dev scripts) with a README. Composition scripts went to their own tracks instead (`D:\Ableton Live projects\<track>\scripts\`, each with a `live.py` copy — self-contained per-track, Option 3). Nothing committed-and-pushed yet; `examples/` is local on kim/main pending Kim's OK to push.
