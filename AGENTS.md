...
On Windows, the optional backend may use `windows-capture` / Windows Graphics Capture so an Ableton Live window can be captured from the compositor even when it is occluded by the agent UI. On macOS, use the OS window-id capture path (`screencapture`/Quartz, or the ScreenCaptureKit `sck` backend) against verified Ableton Live window IDs. Both paths may require the user's OS screen-recording permission. If capture is unavailable, fall back to host status, generated presentation geometry, and web UI telemetry rather than broadening capture permissions.

Embedded Max for Live `jweb`/`jbrowser` (WebView/CEF) device UIs are GPU/IOSurface-backed and, on some macOS versions, can read back blank through the legacy per-window snapshot APIs (`screencapture -l`/Quartz) even while the Max device chrome around them captures fine. `live_visual_capture` handles this automatically: with the default `backend="auto"` on macOS, a blank result is recaptured first with `sck` (window-isolated ScreenCaptureKit), then with a whole-display grab cropped to the Ableton window; the response `backend` field reports which backend produced the saved image and `recovered_from_blank` flags the retry. The display-crop fallback still writes only the Ableton window rectangle, preserving the Ableton-only capture boundary. SCK can fail transiently on some machines (`-3811 "Failed to start stream"` under GPU contention), so the chain falls through to the display crop rather than aborting; `backend="sck"` forces the GPU-surface path directly. Note that on current macOS the legacy default already captures DOM-based jweb panels correctly, so the fallback is a safety net, not the common path.
...
To play the arrangement from a chosen position, use `live_transport` `action: "play_from"` with `time` (beats) — NOT `time` on a `play`/`continue`. Setting `current_song_time` while stopped is ignored (`start_playing` plays from the insert marker, `continue_playing` from the last-stopped cursor), and a same-tick start+jump fails because the jump fires before playback engages. `play_from` starts playback and then runs `song.jump_by(target - current)` on a LATER main-thread tick (via the bridge's `schedule_message` scheduler — never a `time.sleep`, which would block Live's main thread). It returns `deferred_jump: true` immediately with the requested play state; the jump lands a tick or two later, so re-read `live_transport` `action: "status"` in a SEPARATE call to confirm `playing: true` and `current_song_time ≈ time`. `action: "play_loop"` (with `loop_start`/`loop_length`, optional `offset`) sets the arrangement loop brace, enables `song.loop`, and jumps into it for repeatable region playback. `live_record_track_to_wav` chains setup + positioned playback + a self-terminating AgentAudioTap capture (duration from `duration_ms`/`record_bars`, or derived from `region_length`×`repeats`) to record a soloed track's output to a WAV; it is also deferred (`deferred_record: true`) and relies on the tap's record cap to finalize the file.
